...
namespace Visualisation
{
    ...
    {
        public void Draw(HashSet<Tuple<float, float>> obstacles, Transform transform, Material material)
        {
            List<Vector3> obstaclesPositions = processPoints(obstacles);
            renderObstacles(obstaclesPositions, transform, material);
        }

        private List<Vector3> processPoints(HashSet<Tuple<float, float>> obstacles)
        {
